      ...
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
        ...
        {mode === "login" && loginMethod === "code" ? (
          <form onSubmit={codeSent ? verifyLoginCode : (e) => { e.preventDefault(); sendLoginCode(); }} className="mt-6 space-y-4">
            <div>
              <label className={labelCls}>Email</label>
              <div className="relative"><Mail size={16} className="absolute left-3.5 top-3.5 text-slate-400" /><input type="email" disabled={codeSent} className={inputCls + " pl-10"} value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="you@business.co.uk" /></div>
            </div>
            {codeSent && (
              <div>
                <label className={labelCls}>6-digit code</label>
                <div className="relative"><Lock size={16} className="absolute left-3.5 top-3.5 text-slate-400" /><input className={inputCls + " pl-10 tracking-widest"} value={code} onChange={e => setCode(e.target.value)} placeholder="123456" /></div>
            ...
            <button type="submit" disabled={busy} className={btnPrimary + " w-full !py-3.5" + (busy ? " opacity-60" : "")}>{busy ? <Loader2 size={16} className="animate-spin" /> : null}{codeSent ? "Verify & log in" : "Send me a code"}</button>
          </form>
        ) : (
