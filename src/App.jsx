...
async function notifyOwner({ type, subject, lines, photoNote }) {
  ...
  let emailed = false;
  try {
    const res = await fetch("/send-notification-email", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ to: OWNER_EMAIL, subject, text: fullBody }),
    });
    emailed = res.ok;
  } catch (e) {
    emailed = false;
...
function Pricing({ go }) {
  ...
  const submitWaitlist = async (e) => {
    ...
    setWlBusy(true);
    const { mailto, emailed } = await notifyOwner({
    ...
    if (!emailed) window.open(mailto, "_blank");
    setWlBusy(false);
    setWlDone(true);
  };
