...
async function notifyOwner({ type, subject, lines, photoNote }) {
  ...
  let emailed = false;
  try {
    const res = await fetch("/.netlify/functions/send-notification-email", {
      method: "POST",
...
const PRICING_CONFIG = {
  MIN_FEE: 10,
  MAX_FEE: 50,
  BASE_FEE: 10,           // starting fee for a small, simple run
  PER_ITEM_OVER_5: 1.5,   // extra £ per item once past the first 5
...
function calculateRunFee({ itemCount = 0, urgent = false, extrasTotal = 0 }) {
  const c = PRICING_CONFIG;
  const itemSurcharge = Math.max(0, itemCount - 5) * c.PER_ITEM_OVER_5;
  const raw = c.BASE_FEE + itemSurcharge + (urgent ? c.URGENCY_SURCHARGE : 0);
...
function Booking({ go, addOrder, loggedIn, onAuth, account, setAccount, prefillItems, onConsumedPrefill }) {
  ...
  const fee = useMemo(() => {
    const itemCount = data.items.filter(i => i.product.trim()).length;
    return calculateRunFee({ itemCount, urgent: data.urgency === "emergency", extrasTotal });
