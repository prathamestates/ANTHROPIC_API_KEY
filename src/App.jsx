...
import {
  ...
  Gift, Copy, Loader2, Download, Percent, CreditCard, ShieldAlert,
...
/* Real scroll-reveal using IntersectionObserver — an element fades and
   lifts into place the first time it enters the viewport, then stays put
   (never re-hides on scroll-away, so it never fights the user). Reduced
   motion is already handled globally in index.css (all transitions
   collapse to ~0ms), so this component doesn't need its own check — a
   reduced-motion user just sees content appear instantly, not animate. */
...
/* Wraps a grid/row of children, staggering each child's Reveal by a fixed
   step (e.g. cards cascading in one after another instead of all at once). */
function RevealGroup({ children, step = 80, className = "" }) {
  const items = React.Children.toArray(children);
  return (
    <div className={className}>
      {items.map((child, i) => <Reveal key={child.key ?? i} delay={i * step}>{child}</Reveal>)}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <Section className="py-20 bg-white">
        <RevealGroup className="grid md:grid-cols-3 gap-6" step={80}>
          {[
      ...
      <Section id="how" className="py-20 bg-slate-50 scroll-mt-16">
        ...
        <RevealGroup className="grid md:grid-cols-4 gap-6 mb-12" step={90}>
          {[
        ...
        <Reveal className="bg-slate-950 rounded-2xl p-8">
          <PipelineAnimation />
        </Reveal>
      ...
      <Section className="py-20 bg-white">
        ...
        <RevealGroup className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4" step={60}>
...
function AuthPage({ mode, go, onAuth, prefill, referralFromLink }) {
  ...
  // Real validation rules, checked per field so the error points at
  // exactly what's wrong instead of one generic message. UK-format phone
  // check is deliberately loose (Stockly currently only serves UK
  // businesses) — it catches obvious typos without being pedantic about
  // spacing/parentheses people naturally type.
  const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  const UK_PHONE_RE = /^(\+44\s?|0)7\d{3}\s?\d{6}$|^(\+44\s?|0)\d{2,4}\s?\d{3,4}\s?\d{3,4}$/;

  const validateField = (field, value) => {
    switch (field) {
      case "name":
        if (mode === "signup" && !value.trim()) return "Enter your full name.";
        return "";
      case "business":
        if (mode === "signup" && !value.trim()) return "Enter your business name.";
        return "";
      case "email":
      ...
      case "phone":
        if (mode === "signup") {
          ...
          if (!UK_PHONE_RE.test(value.trim())) return "Enter a valid UK phone number, e.g. 07123 456789.";
        }
        return "";
      case "password":
        if (!value) return "Password is required.";
        if (mode === "signup" && value.length < 8) return "Use at least 8 characters.";
        if (mode === "signup" && !/[0-9]/.test(value)) return "Include at least one number.";
        return "";
      default:
        return "";
    }
  ...
  const handleFieldChange = (field, value) => {
    ...
    if (fieldErrors[field]) setFieldErrors(fe => ({ ...fe, [field]: validateField(field, value) }));
  };
  ...
  const submit = async (e) => {
    e.preventDefault();
    setError("");
    const fieldsToCheck = mode === "signup" ? ["name", "business", "email", "phone", "password"] : ["email", "password"];
    const newFieldErrors = {};
    fieldsToCheck.forEach(f => { newFieldErrors[f] = validateField(f, form[f]); });
    setFieldErrors(newFieldErrors);
    if (Object.values(newFieldErrors).some(Boolean)) { setError("Please fix the highlighted fields."); return; }
    setBusy(true);
    try {
      if (mode === "signup") {
    ...
    <div className="min-h-[70vh] bg-slate-50 flex items-center justify-center py-16 px-6">
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
        ...
        <div className="flex mt-6 bg-slate-100 rounded-xl p-1 text-sm">
          <button type="button" onClick={() => go("login")} className={`flex-1 py-2 rounded-lg font-semibold transition ${mode === "login" ? "bg-white shadow-sm text-slate-950" : "text-slate-500"}`}>Sign In</button>
          <button type="button" onClick={() => go("signup")} className={`flex-1 py-2 rounded-lg font-semibold transition ${mode === "signup" ? "bg-white shadow-sm text-slate-950" : "text-slate-500"}`}>Create Account</button>
        </div>
          ...
          {mode === "signup" && (
            <>
              <div>
                ...
                <div className="relative"><User size={16} className="absolute left-3.5 top-3.5 text-slate-400" aria-hidden="true" /><input id="auth-name" autoComplete="name" aria-invalid={!!fieldErrors.name} aria-describedby={fieldErrors.name ? "auth-name-error" : undefined} className={inputCls + " pl-10" + (fieldErrors.name ? " !border-red-300 !ring-red-100" : "")} value={form.name} onChange={e => handleFieldChange("name", e.target.value)} onBlur={() => handleFieldBlur("name")} placeholder="Amir Hussain" /></div>
                {fieldErrors.name && <p id="auth-name-error" className="text-xs text-red-600 mt-1">{fieldErrors.name}</p>}
              </div>
              <div>
                <label htmlFor="auth-business" className={labelCls}>Business name</label>
                <div className="relative"><Building2 size={16} className="absolute left-3.5 top-3.5 text-slate-400" aria-hidden="true" /><input id="auth-business" autoComplete="organization" aria-invalid={!!fieldErrors.business} aria-describedby={fieldErrors.business ? "auth-business-error" : undefined} className={inputCls + " pl-10" + (fieldErrors.business ? " !border-red-300 !ring-red-100" : "")} value={form.business} onChange={e => handleFieldChange("business", e.target.value)} onBlur={() => handleFieldBlur("business")} placeholder="East London Takeaway" /></div>
                {fieldErrors.business && <p id="auth-business-error" className="text-xs text-red-600 mt-1">{fieldErrors.business}</p>}
              </div>
            </>
          )}
          <div>
            <label htmlFor="auth-email" className={labelCls}>Email</label>
            <div className="relative"><Mail size={16} className="absolute left-3.5 top-3.5 text-slate-400" aria-hidden="true" /><input id="auth-email" type="email" autoComplete="email" aria-invalid={!!fieldErrors.email} aria-describedby={fieldErrors.email ? "auth-email-error" : undefined} className={inputCls + " pl-10" + (fieldErrors.email ? " !border-red-300 !ring-red-100" : "")} value={form.email} onChange={e => handleFieldChange("email", e.target.value)} onBlur={() => handleFieldBlur("email")} placeholder="you@business.co.uk" /></div>
            {fieldErrors.email && <p id="auth-email-error" className="text-xs text-red-600 mt-1">{fieldErrors.email}</p>}
          </div>
          {mode === "signup" && (
            <div>
              <label htmlFor="auth-phone" className={labelCls}>Phone</label>
              <div className="relative"><Phone size={16} className="absolute left-3.5 top-3.5 text-slate-400" aria-hidden="true" /><input id="auth-phone" type="tel" autoComplete="tel" aria-invalid={!!fieldErrors.phone} aria-describedby={fieldErrors.phone ? "auth-phone-error" : undefined} className={inputCls + " pl-10" + (fieldErrors.phone ? " !border-red-300 !ring-red-100" : "")} value={form.phone} onChange={e => handleFieldChange("phone", e.target.value)} onBlur={() => handleFieldBlur("phone")} placeholder="07123 456789" /></div>
              {fieldErrors.phone && <p id="auth-phone-error" className="text-xs text-red-600 mt-1">{fieldErrors.phone}</p>}
            </div>
          )}
          <div>
            <label htmlFor="auth-password" className={labelCls}>Password</label>
            <div className="relative"><Lock size={16} className="absolute left-3.5 top-3.5 text-slate-400" aria-hidden="true" /><input id="auth-password" type="password" autoComplete={mode === "signup" ? "new-password" : "current-password"} aria-invalid={!!fieldErrors.password} aria-describedby={fieldErrors.password ? "auth-password-error" : "auth-password-hint"} className={inputCls + " pl-10" + (fieldErrors.password ? " !border-red-300 !ring-red-100" : "")} value={form.password} onChange={e => handleFieldChange("password", e.target.value)} onBlur={() => handleFieldBlur("password")} placeholder="••••••••" /></div>
            {fieldErrors.password ? (
              <p id="auth-password-error" className="text-xs text-red-600 mt-1">{fieldErrors.password}</p>
            ) : mode === "signup" && (
              <p id="auth-password-hint" className="text-[11px] text-slate-400 mt-1">At least 8 characters, including a number.</p>
            )}
          </div>
          {mode === "signup" && (
            <div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 stk-fade-up" key={step}>
          ...
          {step === 5 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-slate-500 mb-5">Type your list, or ask Stockly to prepare it for you.</p>
              ...
              {itemMode === "assistant" && (
                <div className="rounded-xl border border-slate-200 p-4 mb-4 bg-slate-50">
                  ...
                  {assistantError && (
                    <div className="mt-3 bg-red-50 border border-red-200 rounded-lg px-3 py-2.5 text-xs text-red-600">
                      {assistantError}
                      <div className="mt-1.5"><button type="button" onClick={() => setItemMode("manual")} className="font-semibold underline underline-offset-2">Type your list manually instead</button></div>
                    </div>
                  )}
...
function DashLayout({ active, setActive, children, go, title = "Dashboard", customer, onLogout, myOrders = [] }) {
...
function CustomerDashboard({ go, account, setAccount, onLogout, setBookingPrefill }) {
  ...
  return (
    <DashLayout active={tab} setActive={setTab} go={go} title="Dashboard" customer={account?.full_name || account?.business_name || "Account"} onLogout={onLogout} myOrders={myOrders}>
      {tab === "Dashboard" && (
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: listener } = supabase.auth.onAuthStateChange(async (event, newSession) => {
        ...
        // tab wakes up) should NOT wipe a real login. A genuine expiry
        // will still be caught the next time something actually needs
        // the session (e.g. submitting a run re-checks with getUser()).
        if (event === "SIGNED_OUT") {
