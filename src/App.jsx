...
import {
  ...
  LayoutDashboard, HelpCircle, Banknote
} from "lucide-react";
...
import {
  fetchAllRuns, fetchMyAssignedRuns, fetchOpsTeam, assignRun,
  updateRunStatus, setRunFlag, buildOpsStats,
} from "./lib/opsData";
...
// Ops access is controlled by flags on profiles (is_admin / is_shopper /
// is_driver), enforced by Supabase RLS — there is intentionally NO
// passcode in this bundle. Grant access in Supabase, e.g.:
//   update profiles set is_admin = true where email = 'you@...';
// See supabase/migrations/0007-ops-real-dashboards.sql.
...
// Ops dashboards are gated by profile flags, not by anything client-side
// secret — the flags are enforced again at the database by RLS.
function roleFromProfile(p) {
  return p?.is_admin ? "admin" : p?.is_shopper ? "shopper" : p?.is_driver ? "driver" : "customer";
}

/* Records a new signup / order / waitlist submission into local storage so
   it appears on this device's admin dashboard, and sends a real email to
   the owner via the send-notification-email function (Resend). If that
   isn't set up yet (no RESEND_API_KEY), it falls back to opening a
   ready-to-send email draft so nothing gets silently lost. */
async function notifyOwner({ type, subject, lines, photoNote }) {
  ...
  // WhatsApp's "click to chat" link opens WhatsApp with this message
  // pre-filled, ready for the customer to send with one tap. There's no
  // way to send a WhatsApp message with zero taps from a plain website —
  // that requires a paid WhatsApp Business API integration — so this is
...
// stock — it is separate from the price of the products themselves,
// which the business pays the cash & carry directly (see PAYMENT_NOTE
// below). Every number here is a named constant so it's easy to tune
// later without hunting through the calculation logic.
// ---------------------------------------------------------------------
const PRICING_CONFIG = {
  ...
  PER_UNIT: 0.4,          // extra £ per unit past the free amount — this is what makes large quantities of one product cost more, not just many different products
...
// not just displayed. Tune these numbers freely.
...
/** Transparent, configurable Stockly service-fee calculation.
 * Factors in both how many different products are on the list (itemCount)
 * and how much is actually being bought (totalUnits — the summed
 * quantities), so a run for 50 of one product costs more than a run for 1,
 * not just runs with more distinct product lines.
 * Always returns a number between PRICING_CONFIG.MIN_FEE and MAX_FEE. */
function calculateRunFee({ itemCount = 0, totalUnits = 0, urgent = false, extrasTotal = 0 }) {
  ...
  return rounded + extrasTotal; // extras (priority collection etc.) are separate opt-ins, added after clamping
}
...
/* A simple visual progress tracker along STATUS_FLOW, so a customer can see
   at a glance how far along their run is instead of just reading a word. */
      ...
      {STATUS_FLOW.map((s, i) => (
        <React.Fragment key={s}>
          <div className="flex flex-col items-center flex-shrink-0" style={{ width: 64 }}>
            <div className={`w-3 h-3 rounded-full ${i <= idx ? "bg-lime-400" : "bg-slate-200"}`} />
            <span className={`text-[9px] mt-1.5 text-center leading-tight ${i <= idx ? "text-slate-700 font-semibold" : "text-slate-400"}`}>{s}</span>
          </div>
          {i < STATUS_FLOW.length - 1 && <div className={`h-0.5 flex-1 -mt-4 ${i < idx ? "bg-lime-400" : "bg-slate-200"}`} />}
        </React.Fragment>
      ))}
    </div>
  );
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
        {deals.map((d, i) => (
          <div key={i} className="bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-md transition">
            <img src={d.image_url} alt={`${d.supplier_name} weekly deals`} className="w-full h-48 object-cover bg-slate-100" />
            <div className="p-4">
    ...
    <div className="bg-white">
      {waitlistPlan && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 flex items-end sm:items-center justify-center p-0 sm:p-6" onClick={() => { setWaitlistPlan(null); setWlDone(false); }}>
          <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-md p-6 sm:p-8" onClick={e => e.stopPropagation()}>
            {wlDone ? (
              ...
              <>
                ...
                <form onSubmit={submitWaitlist} className="mt-5 space-y-3">
                  <input className={inputCls} placeholder="Your name" value={wlForm.name} onChange={e => setWlForm({ ...wlForm, name: e.target.value })} />
                  <input className={inputCls} placeholder="Business name (optional)" value={wlForm.business} onChange={e => setWlForm({ ...wlForm, business: e.target.value })} />
                  <input type="email" className={inputCls} placeholder="Email" value={wlForm.email} onChange={e => setWlForm({ ...wlForm, email: e.target.value })} />
                  <input className={inputCls} placeholder="Phone (optional)" value={wlForm.phone} onChange={e => setWlForm({ ...wlForm, phone: e.target.value })} />
                  {wlError && <p className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{wlError}</p>}
...
function AuthPage({ mode, go, onAuth, prefill, referralFromLink }) {
  ...
  // of typing a password. Uses Supabase's built-in email OTP — no extra
  // service or cost, but Email OTP must be left enabled in Supabase
  // (Authentication > Providers > Email) for this to send.
  ...
  return (
    <div className="min-h-[70vh] bg-slate-50 flex items-center justify-center py-16 px-6">
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
        ...
        {mode === "login" && (
          <div className="flex mt-6 bg-slate-100 rounded-xl p-1 text-sm">
            <button type="button" onClick={() => { setLoginMethod("password"); setCodeSent(false); setError(""); }} className={`flex-1 py-2 rounded-lg font-semibold transition ${loginMethod === "password" ? "bg-white shadow-sm text-slate-950" : "text-slate-500"}`}>Password</button>
            <button type="button" onClick={() => { setLoginMethod("code"); setError(""); }} className={`flex-1 py-2 rounded-lg font-semibold transition ${loginMethod === "code" ? "bg-white shadow-sm text-slate-950" : "text-slate-500"}`}>Email code</button>
          </div>
        )}
        ...
        <div className="border-t border-slate-100 mt-6 pt-5 text-center">
          <p className="text-xs text-slate-400">Stockly team? Head to the <button onClick={() => go("admin")} className="font-semibold text-slate-600 underline underline-offset-2">ops dashboard</button>.</p>
        </div>
      </div>
    </div>
  );
...
/* Sends a photo to a Cloudflare Pages Function (functions/recognize-items.js)
...
async function recognizeItemsFromPhoto(base64Data, mediaType) {
  const response = await fetch("/recognize-items", {
    method: "POST",
...
function Booking({ go, addOrder, loggedIn, onAuth, account, setAccount, prefillItems, onConsumedPrefill }) {
  ...
  // prefill it. (Kept in localStorage rather than the database since it's
  // just a convenience default, not something that needs to sync across
  // devices or be queried elsewhere.)
  ...
  const [step, setStep] = useState(() => {
    ...
    // the returning customer's time is worth more than re-confirming
  ...
  const [itemMode, setItemMode] = useState("manual"); // "manual" | "photo" | "barcode" | "assistant"
  ...
  // from this customer's actual past runs, never fabricated. If they have
  // no history yet, the assistant is told that explicitly (see
  // functions/ai-assistant.js) rather than pretending to know a "usual".
  ...
  useEffect(() => {
    ...
    fetchSingleRuns(account.id)
      .then(runs => setOrderHistory((runs || []).slice(0, 10).map(r => ({ date: r.date, cashAndCarry: r.cashAndCarry, items: r.items }))))
  ...
  // one item per line into real item rows, so a busy owner can just type
  // (or paste from Notes/WhatsApp) instead of filling in a form field by
  // field. Deliberately simple pattern matching, not AI — a leading number
  // is read as the quantity, the rest as the product name.
  ...
  // waitlist (emails the Stockly team the customer's real details) rather
  // than pretending to activate a paid plan with no payment behind it.
  ...
  const joinMembershipWaitlist = async (plan) => {
    ...
    if (!activeAccount?.id) {
      ...
      // has been backgrounded a while that can be stale. getUser() actively
      // re-checks with Supabase's servers and refreshes the token if needed.
  ...
  const submit = async () => {
    ...
    setSubmitError("");
    let activeAccount = account;
    let diag = `prop-account=${account ? "present" : "MISSING"}`;
    if (!activeAccount?.id) {
      // getSession() only reads whatever's cached locally — after the tab
      ...
      // and refreshes the token if needed, so it catches cases getSession()
      // would incorrectly treat as "logged out".
      const { data: { user }, error: userErr } = await supabase.auth.getUser();
      diag += ` | getUser=${user ? "found:" + user.id.slice(0, 8) : "NULL"}${userErr ? ` err:${userErr.message}` : ""}`;
      if (user) {
        const { data: profile, error: profileErr } = await supabase.from("profiles").select("*").eq("id", user.id).single();
        diag += ` | profile=${profile ? "found" : "MISSING"}${profileErr ? ` err:${profileErr.message}` : ""}`;
        if (profile) { activeAccount = profile; setAccount(profile); }
      }
    }
    if (!activeAccount?.id) {
      setSubmitError(`You've been logged out — please log back in and submit again. [debug: ${diag}]`);
      return;
    }
    setSubmitting(true);
    let order;
    ...
    return (
      <div className="min-h-[70vh] bg-slate-50 flex items-center justify-center px-6 py-20">
        <div className="max-w-md w-full bg-white rounded-2xl border border-slate-200 p-8 text-center stk-fade-up">
          ...
          <p className="text-xs text-slate-400 mt-4">A WhatsApp message with your order has opened, ready to send — if it didn't open, message or call <a href={`tel:${OWNER_PHONE_TEL}`} className="font-semibold text-slate-600">{OWNER_PHONE_DISPLAY}</a> directly and we'll confirm your order.</p>
          <div className="flex flex-col gap-2.5 mt-7">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 stk-fade-up" key={step}>
          ...
          {step === 5 && (
            <div className="space-y-4">
              ...
              <div className="flex gap-2 p-1 bg-slate-100 rounded-xl mb-4 flex-wrap">
                ...
                <button type="button" onClick={() => setItemMode("assistant")} className={`flex-1 py-2 rounded-lg text-sm font-semibold transition ${itemMode === "assistant" ? "bg-white text-slate-950 shadow-sm" : "text-slate-500"}`}><Sparkles size={14} className="inline mr-1.5 -mt-0.5" /> Ask Stockly</button>
              </div>
              ...
              {itemMode === "assistant" && (
                <div className="rounded-xl border border-slate-200 p-4 mb-4 bg-slate-50">
                  ...
                  <div className="flex justify-between items-center mt-2">
                    ...
                    <button type="button" onClick={runAssistant} disabled={!assistantText.trim() || assistantBusy} className={btnDark + " !py-2 !px-3.5 text-xs disabled:opacity-40"}>{assistantBusy ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} />}{assistantBusy ? "Thinking…" : "Ask"}</button>
                  </div>
                  {assistantNote && (
                  ...
                  {assistantError && (
                    <div className="mt-3 bg-red-50 border border-red-200 rounded-lg px-3 py-2.5 text-xs text-red-600">{assistantError}</div>
                  )}
                  <p className="text-[11px] text-slate-400 mt-2">Nothing is added to your run until you review it below and confirm at the end — the assistant only prepares a draft.</p>
                </div>
              )}
              ...
              {itemMode === "photo" && (
                <div className="rounded-xl border-2 border-dashed border-slate-300 p-6 text-center mb-4">
                  ...
                  {photoError && (
                    <div className="mt-3 text-left bg-red-50 border border-red-200 rounded-lg px-3 py-2.5">
                      <p className="text-xs text-red-600">{photoError}</p>
                      <div className="flex flex-wrap gap-3 mt-2">
                        <button type="button" onClick={() => fileInputRef.current?.click()} className="text-xs font-semibold text-slate-900 underline underline-offset-2">Try again</button>
                        <button type="button" onClick={() => setItemMode("barcode")} className="text-xs font-semibold text-slate-900 underline underline-offset-2">Scan barcode instead</button>
                        <button type="button" onClick={() => setItemMode("manual")} className="text-xs font-semibold text-slate-900 underline underline-offset-2">Enter manually</button>
                      </div>
                    </div>
                  )}
                </div>
          ...
          {step === 6 && (
            <div className="space-y-5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mb-2.5">Membership billing isn't live yet. Picking a plan here applies it to this order only — press "Join waitlist" on a plan to be contacted when it's ready.</p>
                <div className="grid sm:grid-cols-2 gap-2.5">
                  {MEMBERSHIP_PLANS.map(p => {
                    ...
                    <div key={p.id} className={`text-left px-4 py-3 rounded-xl border transition ${data.membership === p.id ? "border-lime-400 bg-lime-50" : "border-slate-200 hover:border-slate-300"}`}>
                      <button type="button" onClick={() => set({ membership: p.id })} className="w-full text-left">
                ...
                {membershipError && <p className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2 mt-2.5">{membershipError}</p>}
          ...
          {step === 8 && (
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-slate-200 divide-y divide-slate-100 text-sm">
                ...
                <div className="flex justify-between px-4 py-3"><span className="text-slate-500">Date</span><span className="font-medium text-slate-900">{data.date || "—"}{data.timeWindow ? ` · ${data.timeWindow}` : ""}</span></div>
                ...
                {(firstRunDiscountAmount > 0 || creditApplied > 0) ? (
                  <>
                    <div className="flex justify-between px-4 py-3"><span className="text-slate-500">Subtotal</span><span className="text-slate-700">£{preDiscountFee}</span></div>
                    {firstRunDiscountAmount > 0 && (
                      <div className="flex justify-between px-4 py-3"><span className="text-lime-700">First-run discount ({FIRST_RUN_DISCOUNT_PERCENT}% off)</span><span className="text-lime-700">−£{firstRunDiscountAmount}</span></div>
                    )}
                    {creditApplied > 0 && (
                      <div className="flex justify-between px-4 py-3"><span className="text-lime-700">Referral credit applied</span><span className="text-lime-700">−£{creditApplied}</span></div>
                    )}
                    <div className="flex justify-between px-4 py-3 border-t border-slate-100"><span className="font-semibold text-slate-900">Total fee</span><span className="font-bold text-slate-950">£{fee}</span></div>
                  </>
                ) : (
              ...
              <p className="text-xs text-slate-400">Submitting will open WhatsApp with your full order, including quantities and address, ready to send to the Stockly team — and save it to your dashboard.</p>
...
function DashLayout({ active, setActive, children, go, title = "Dashboard", customer, customerId, onLogout, myOrders = [] }) {
  const [notifOpen, setNotifOpen] = useState(false);
  const [menuOpen, setMenuOpen] = useState(false);
  // Real notifications from the `notifications` table (written by the
  // log_run_event trigger whenever ops staff change a run's status).
  // Falls back to order-derived updates until migration 0007 is run.
  ...
  }, [customerId, myOrders.length]);
  const derivedNotifs = [...myOrders]
    .sort((a, b) => (b.date || "").localeCompare(a.date || ""))
    .slice(0, 5)
    .map(o => ({
      id: `run-${o.dbId}`,
      title: `${o.status}`,
      body: `${o.id || o.dbId} · ${o.cashAndCarry || "Stock run"}`,
      time: o.date || "",
      unread: false,
    }));
  const notifs = dbNotifs.length
    ? dbNotifs.map(n => ({
        id: n.id,
        title: n.title,
        body: n.body || "",
        time: n.created_at ? new Date(n.created_at).toLocaleString("en-GB", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }) : "",
        unread: !n.read,
      }))
    : derivedNotifs;
  ...
  const toggleNotifs = async () => {
    ...
    if (next && unreadCount > 0) {
      setDbNotifs(ns => ns.map(n => ({ ...n, read: true })));
      await supabase.from("notifications").update({ read: true })
        .eq("customer_id", customerId).eq("read", false);
    }
  };
  const pick = (item) => {
    setMenuOpen(false);
        ...
        <Section className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <button onClick={toggleNotifs} aria-label="Notifications" className="p-2 rounded-lg hover:bg-slate-100 relative">
                <Bell size={18} className="text-slate-600" />
                {unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-1 bg-lime-400 text-slate-950 text-[10px] font-bold rounded-full flex items-center justify-center">{unreadCount}</span>
                )}
              </button>
              {notifOpen && (
                <div className="absolute right-0 mt-2 w-72 bg-white border border-slate-200 rounded-xl shadow-lg p-2 stk-fade-up z-20">
                  ...
                  {notifs.map(n => (
                    <div key={n.id} className={`p-3 rounded-lg hover:bg-slate-50 ${n.unread ? "bg-lime-50/60" : ""}`}>
...
function CustomerDashboard({ go, account, setAccount, onLogout, setBookingPrefill }) {
  ...
  // demo/placeholder figures. Only counts orders that actually have a date
  // (i.e. real rows from Supabase).
  ...
  return (
    <DashLayout active={tab} setActive={setTab} go={go} title="Dashboard" customer={account?.full_name || account?.business_name || "Account"} customerId={account?.id} onLogout={onLogout} myOrders={myOrders}>
      {tab === "Dashboard" && (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{myOrders[0].cashAndCarry} · {(myOrders[0].items || []).length} items · {myOrders[0].date}</p>
                </div>
              </div>
            ...
            <div className="space-y-3">
              {realTopProducts.map((p, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="w-32 h-2 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-lime-400" style={{ width: `${(p.count / (realTopProducts[0]?.count || 1)) * 100}%` }} /></div>
      ...
      {tab === "Referrals" && (
        <div className="space-y-6">
          <div className="bg-slate-950 rounded-2xl p-7">
            ...
            <p className="text-slate-400 text-sm mt-2 max-w-md">Share your code. When someone signs up with it and completes their first run, you get £{REFERRAL_REWARD_AMOUNT} credit automatically — it's applied straight to their next order's fee for you.</p>
...
--------------------------------------------------------------------- */
/* ---------------------------------------------------------------------
   OPS CONSOLE (admin / shopper / driver)
   Access is enforced twice: by role here, and by RLS in migration 0007.
--------------------------------------------------------------------- */
function OpsHeader({ go, onLogout, who }) {
  return (
    <div className="bg-slate-950 border-b border-slate-800">
      <Section className="flex items-center justify-between h-16">
        <button onClick={() => go("home")}><Logo dark size={26} /></button>
        <div className="flex items-center gap-4">
          <span className="hidden sm:inline text-xs text-slate-400">{who}</span>
          <a href={`tel:${OWNER_PHONE_TEL}`} className="hidden md:flex items-center gap-1.5 text-sm text-slate-300 hover:text-white"><Phone size={14} /> {OWNER_PHONE_DISPLAY}</a>
          <button onClick={onLogout} className="text-slate-300 hover:text-white flex items-center gap-1.5 text-sm"><LogOut size={16} /> Log out</button>
        </div>
      </Section>
    </div>
  );
  ...
  return (
    <div className="min-h-[80vh] bg-slate-50 flex items-center justify-center px-6">
      <div className="max-w-md w-full bg-white border border-slate-200 rounded-2xl p-8 text-center">
        <ShieldAlert className="mx-auto text-slate-400 mb-4" size={28} />
        <h1 className="font-display font-bold text-xl text-slate-950">Team sign-in required</h1>
        <p className="text-slate-500 text-sm mt-2">This area is for the Stockly operations team. Sign in with a team account that has the right role, or head back to the site.</p>
        <div className="flex gap-2 justify-center mt-6">
          <button onClick={() => go("login")} className={btnGhost}>Log in</button>
          <button onClick={() => go("home")} className={btnDark}>Back to site</button>
        </div>
      </div>
    </div>
  );
...
function AdminDashboard({ go, account, onLogout }) {
  ...
  // Website submissions (signups / orders with no account yet) still land
  // in device storage via notifyOwner — that's the only localStorage left.
  useEffect(() => { (async () => setNotifs((await storageGet("owner-notifications", true)) || []))(); }, []);

  const reload = async () => {
    setLoading(true); setError("");
    try {
      const [all, opsTeam] = await Promise.all([fetchAllRuns(), fetchOpsTeam()]);
      setRuns(all); setTeam(opsTeam);
    } catch (err) {
      setError(err.message || "Couldn't load runs from the database.");
    } finally { setLoading(false); }
  };
  useEffect(() => { if (account?.id) reload(); }, [account?.id]);

  if (!account) return <OpsDenied go={go} />;
  if (loading) return (<div><OpsHeader go={go} onLogout={onLogout} who="Stockly · Admin" /><OpsLoading /></div>);

  const shoppers = team.filter(t => t.is_shopper);
  const drivers = team.filter(t => t.is_driver);
  const stats = buildOpsStats(runs);

  const step = async (run, dir) => {
    ...
    setBusyId(run.dbId);
    try {
      await updateRunStatus(run.dbId, run.status, next);
      await reload();
    } catch (err) { setError(err.message || "Couldn't update that run."); }
    finally { setBusyId(null); }
  };

  const assign = async (run, field, value) => {
    setBusyId(run.dbId);
    try {
      await assignRun(run.dbId, field === "shopper" ? { shopperId: value || null } : { driverId: value || null });
      await reload();
    } catch (err) { setError(err.message || "Couldn't save that assignment."); }
    finally { setBusyId(null); }
  };

  return (
    <div className="bg-slate-50 min-h-[80vh]">
      <OpsHeader go={go} onLogout={onLogout} who={`${account.full_name || account.email} · Admin`} />
      <Section className="py-8">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <div>
            <h1 className="font-display font-bold text-2xl text-slate-950">Operations board</h1>
            <p className="text-sm text-slate-500">{stats.active} active · {stats.requested} awaiting confirmation · {stats.unassigned} unassigned</p>
          </div>
          ...
          {STATUS_FLOW.map(status => (
            <div key={status} className="min-w-[280px] w-[280px] shrink-0">
              <div className="flex items-center justify-between mb-3 px-1">
                ...
                <span className="text-xs text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{runs.filter(o => o.status === status).length}</span>
              </div>
              <div className="space-y-3">
                {runs.filter(o => o.status === status).map(o => (
                  <div key={o.dbId} className={`bg-white rounded-xl border p-4 ${busyId === o.dbId ? "border-lime-300" : "border-slate-200"}`}>
                    <div className="flex items-center justify-between">
                      <p className="font-semibold text-sm text-slate-900">{o.id}</p>
                      <span className="text-xs text-slate-400">£{o.serviceFee.toFixed(2)}</span>
                    </div>
                    <p className="text-xs text-slate-700 mt-1 font-medium">{o.businessName || "Stockly customer"}</p>
                    <p className="text-xs text-slate-500 mt-0.5">{o.cashAndCarry}</p>
                    <p className="text-xs text-slate-400 mt-0.5 truncate">{o.address}</p>
                    <div className="flex flex-wrap gap-1 mt-2.5">
                      <select value={o.assignedShopperId || ""} disabled={busyId === o.dbId} onChange={e => assign(o, "shopper", e.target.value)} className="text-[11px] border border-slate-200 rounded-md px-1.5 py-1 text-slate-600 max-w-[120px]">
                        <option value="">Shopper</option>
                        {shoppers.map(s => <option key={s.id} value={s.id}>{s.full_name || s.email}</option>)}
                      </select>
                      <select value={o.assignedDriverId || ""} disabled={busyId === o.dbId} onChange={e => assign(o, "driver", e.target.value)} className="text-[11px] border border-slate-200 rounded-md px-1.5 py-1 text-slate-600 max-w-[120px]">
                        <option value="">Driver</option>
                        {drivers.map(d => <option key={d.id} value={d.id}>{d.full_name || d.email}</option>)}
                      </select>
                    </div>
                    <div className="flex gap-1.5 mt-3">
                      <button onClick={() => step(o, -1)} disabled={busyId === o.dbId} className="flex-1 text-xs py-1.5 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 disabled:opacity-40"><ChevronLeft size={12} className="inline" /></button>
                      <button onClick={() => step(o, 1)} disabled={busyId === o.dbId} className="flex-1 text-xs py-1.5 rounded-lg bg-slate-950 text-white hover:bg-slate-800 disabled:opacity-40">{busyId === o.dbId ? "Saving…" : <>Next <ChevronRight size={12} className="inline" /></>}</button>
                    </div>
                  </div>
                ))}
                {runs.filter(o => o.status === status).length === 0 && (
                  <div className="text-xs text-slate-300 border border-dashed border-slate-200 rounded-xl p-6 text-center">No runs</div>
                )}
              </div>
            </div>
          ))}
...
/* ---------------------------------------------------------------------
   SHOPPER DASHBOARD
--------------------------------------------------------------------- */
function ShopperDashboard({ go, account, onLogout }) {
  const [runs, setRuns] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [openDbId, setOpenDbId] = useState(null);
  const [checked, setChecked] = useState({});
  const [busy, setBusy] = useState(false);

  const reload = async () => {
    setLoading(true); setError("");
    try { setRuns(await fetchMyAssignedRuns(account.id, "shopper")); }
  ...
  const myRuns = runs.filter(o => !["Delivered", "Cancelled"].includes(o.status));
  const openRun = runs.find(r => r.dbId === openDbId) || null;

  const advance = async (run, toStatus) => {
    setBusy(true); setError("");
    try { await updateRunStatus(run.dbId, run.status, toStatus); await reload(); }
    catch (err) { setError(err.message || "Couldn't update that run."); }
    finally { setBusy(false); }
  };

  const toggleReceipt = async (run) => {
    setBusy(true); setError("");
    try { await setRunFlag(run.dbId, "receipt", !run.receiptUploaded); await reload(); }
    catch (err) { setError(err.message || "Couldn't save the receipt flag."); }
    finally { setBusy(false); }
  };

  return (
    <div className="bg-slate-50 min-h-[80vh]">
      <OpsHeader go={go} onLogout={onLogout} who={`${account.full_name || account.email} · Shopper`} />
      {loading ? <OpsLoading /> : (
      <Section className="py-6 max-w-lg mx-auto">
        {error && <p className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2 mb-4">{error}</p>}
        {!openRun ? (
          <>
            <h1 className="font-display font-bold text-2xl text-slate-950 mb-1">My Stock Runs</h1>
            <p className="text-sm text-slate-500 mb-6">{myRuns.length} assigned</p>
            <div className="space-y-3">
              {myRuns.map(o => (
                <button key={o.dbId} onClick={() => setOpenDbId(o.dbId)} className="w-full text-left bg-white rounded-2xl border border-slate-200 p-5 hover:border-slate-300 transition">
                  <div className="flex items-center justify-between">
                    <p className="font-semibold text-slate-900">{o.id}</p>
                    <StatusPill status={o.status} />
                  </div>
                  <p className="text-sm text-slate-700 mt-2 font-medium">{o.businessName || "Stockly customer"}</p>
              ...
              {myRuns.length === 0 && <p className="text-sm text-slate-400 text-center py-10">No stock runs assigned to you right now.</p>}
            </div>
          </>
        ) : (
          <div>
            <button onClick={() => setOpenDbId(null)} className="text-sm text-slate-500 flex items-center gap-1 mb-4"><ChevronLeft size={16} /> Back</button>
            <div className="bg-white rounded-2xl border border-slate-200 p-5 mb-4">
              <div className="flex items-center justify-between">
                <h2 className="font-display font-bold text-xl text-slate-950">{openRun.id}</h2>
                <StatusPill status={openRun.status} />
              </div>
              <p className="text-sm text-slate-700 mt-1 font-medium">{openRun.businessName || "Stockly customer"}</p>
              <p className="text-sm text-slate-500 mt-0.5">{openRun.cashAndCarry}</p>
              {openRun.notes && <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mt-3">{openRun.notes}</p>}
            </div>
            <div className="bg-white rounded-2xl border border-slate-200 p-5 mb-4">
              <h3 className="font-semibold text-sm text-slate-700 mb-3">Checklist</h3>
              <div className="space-y-2.5">
                {openRun.items.map((it, i) => {
                  const key = `${openRun.dbId}-${i}`;
                  return (
                    <button key={i} onClick={() => setChecked(c => ({ ...c, [key]: !c[key] }))} className="w-full flex items-center gap-3 text-left">
            ...
            <div className="flex flex-col gap-2.5">
              {openRun.status === "Confirmed" && <button onClick={() => advance(openRun, "Purchasing")} disabled={busy} className={btnPrimary + " disabled:opacity-50"}><PackageSearch size={16} /> {busy ? "Saving…" : "Start shopping"}</button>}
              {openRun.status === "Purchasing" && <button onClick={() => advance(openRun, "Collected")} disabled={busy} className={btnPrimary + " disabled:opacity-50"}><PackageCheck size={16} /> {busy ? "Saving…" : "Mark collected"}</button>}
              <button onClick={() => toggleReceipt(openRun)} disabled={busy} className={btnGhost + " disabled:opacity-50"}><Upload size={16} /> {openRun.receiptUploaded ? "Receipt uploaded ✓ (tap to undo)" : "Confirm receipt photographed"}</button>
            </div>
          </div>
        )}
      </Section>
...
/* ---------------------------------------------------------------------
   DRIVER DASHBOARD
--------------------------------------------------------------------- */
function DriverDashboard({ go, account, onLogout }) {
  const [runs, setRuns] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [busyId, setBusyId] = useState(null);
  ...
  const reload = async () => {
    setLoading(true); setError("");
    try { setRuns(await fetchMyAssignedRuns(account.id, "driver")); }
  ...
  const myRuns = runs.filter(o => ["Collected", "Out for Delivery"].includes(o.status));

  const advance = async (o) => {
    const next = o.status === "Collected" ? "Out for Delivery" : "Delivered";
    setBusyId(o.dbId); setError("");
    try {
      await updateRunStatus(o.dbId, o.status, next);
      if (next === "Delivered") await setRunFlag(o.dbId, "pod", true);
      await reload();
    } catch (err) { setError(err.message || "Couldn't update that delivery."); }
    finally { setBusyId(null); }
  };

  return (
    <div className="bg-slate-50 min-h-[80vh]">
      <OpsHeader go={go} onLogout={onLogout} who={`${account.full_name || account.email} · Driver`} />
      {loading ? <OpsLoading /> : (
      <Section className="py-6 max-w-lg mx-auto">
        <h1 className="font-display font-bold text-2xl text-slate-950 mb-1">Deliveries</h1>
        <p className="text-sm text-slate-500 mb-6">{myRuns.length} assigned to you</p>
        {error && <p className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2 mb-4">{error}</p>}
        <div className="space-y-4">
          {myRuns.map(o => (
            <div key={o.dbId} className="bg-white rounded-2xl border border-slate-200 p-5">
              <div className="flex items-center justify-between">
                <p className="font-semibold text-slate-900">{o.id}</p>
                <StatusPill status={o.status} />
              </div>
              ...
              <p className="text-xs text-slate-500 mt-1">{o.items.length} products{o.podUploaded ? " · POD captured" : ""}</p>
              ...
              <div className="flex gap-2.5 mt-4">
                ...
                <button onClick={() => advance(o)} disabled={busyId === o.dbId} className={btnPrimary + " flex-1 !py-2.5 text-xs disabled:opacity-50"}>{busyId === o.dbId ? "Saving…" : o.status === "Collected" ? "Start delivery" : "Mark delivered"}</button>
              </div>
            </div>
          ))}
          {myRuns.length === 0 && <p className="text-sm text-slate-400 text-center py-10">No deliveries assigned right now.</p>}
        </div>
      </Section>
      )}
    </div>
  ...
  if (title === "Privacy Policy") return (
    <div className={wrap}>
      ...
      <LegalSection heading="How it's used">
        <p>Your order details are sent, via WhatsApp, to the Stockly team so your run can be collected and delivered. We don't sell your data or share it with anyone beyond what's needed to fulfil your order.</p>
      </LegalSection>
...
export default function StocklyApp() {
  ...
  // Legacy sink for the booking wizard's confirmation callback — the ops
  // dashboards no longer read this; they fetch from Supabase (opsData.js).
  const [orders, setOrders] = useState([]);
  const [bookingPrefill, setBookingPrefill] = useState(null);

  // Restore a logged-in session from Supabase's real auth session when the
  // app reopens. Role comes from the profile flags (RLS-enforced).
  useEffect(() => {
    let unsub;
    (async () => {
      ...
      if (session?.user) {
        ...
        if (profile) { setAccount(profile); setLoggedIn(true); setRole(roleFromProfile(profile)); }
      }
      const { data: listener } = supabase.auth.onAuthStateChange(async (event, newSession) => {
        // Only an explicit, real sign-out counts as a sign-out — token
        // refresh hiccups shouldn't wipe a genuine login.
        if (event === "SIGNED_OUT") {
          setAccount(null); setLoggedIn(false); setRole("customer");
          return;
        }
        if (newSession?.user) {
  ...
  // Role always comes from the profile flags after login — never from a
  // client-chosen dropdown — so an ops login lands on the right board.
  const onAuth = async (r, acc) => {
    const effective = acc ? roleFromProfile(acc) : r;
    setLoggedIn(true); setRole(effective);
    if (acc) setAccount(acc);
    go(effective === "customer" ? "dashboard" : effective);
  };
  ...
  else if (page === "book") content = <Booking go={go} addOrder={addOrder} loggedIn={loggedIn} onAuth={onAuth} account={account} setAccount={setAccount} prefillItems={bookingPrefill} onConsumedPrefill={() => setBookingPrefill(null)} />;
  ...
  // Ops consoles are gated by role (which comes from RLS-backed profile
  // flags). Anyone else gets the team sign-in screen.
  else if (page === "admin") content = role === "admin" ? <AdminDashboard go={go} account={account} onLogout={onLogout} /> : <OpsDenied go={go} />;
  else if (page === "shopper") content = (role === "shopper" || role === "admin") ? <ShopperDashboard go={go} account={account} onLogout={onLogout} /> : <OpsDenied go={go} />;
  else if (page === "driver") content = (role === "driver" || role === "admin") ? <DriverDashboard go={go} account={account} onLogout={onLogout} /> : <OpsDenied go={go} />;
