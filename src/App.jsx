...
function Booking({ go, addOrder, loggedIn, onAuth, account, setAccount, prefillItems, onConsumedPrefill }) {
  ...
  const submit = async () => {
    ...
    let activeAccount = account;
    if (!activeAccount?.id) {
      // The app's copy of "who's logged in" can occasionally lag behind
      // Supabase's real session (e.g. after a token refresh). Before
      // giving up, actively re-check the real session rather than trusting
      // a possibly-stale prop.
      const { data: { session } } = await supabase.auth.getSession();
      if (session?.user) {
        const { data: profile } = await supabase.from("profiles").select("*").eq("id", session.user.id).single();
        if (profile) { activeAccount = profile; setAccount(profile); }
