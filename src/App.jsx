...
function Booking({ go, addOrder, loggedIn, onAuth, account, setAccount, prefillItems, onConsumedPrefill }) {
  ...
  // "Start Membership" is a real, separate action from just picking a plan
  // for this one order — pressing it actually activates membership on the
  // account right away (saved to profiles.membership in Supabase), rather
  // than only taking effect once the run is submitted.
  const [membershipActivating, setMembershipActivating] = useState(false);
  const [membershipError, setMembershipError] = useState("");
  const startMembership = async (planId) => {
    setMembershipError("");
    ...
    if (!activeAccount?.id) { go("login"); return; }
    setMembershipActivating(true);
    try {
      const updated = await updateProfile(activeAccount.id, { membership: planId });
      setAccount(updated);
    } catch (err) {
          ...
          {step === 6 && (
            <div className="space-y-5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mb-2.5">Picking a plan here applies it to this order only. Press "Start membership" on a plan to activate it on your account from today.</p>
                <div className="grid sm:grid-cols-2 gap-2.5">
                  {MEMBERSHIP_PLANS.map(p => {
                    const isActiveOnAccount = account?.membership === p.id && p.id !== "none";
                    return (
                    <div key={p.id} className={`text-left px-4 py-3 rounded-xl border transition ${data.membership === p.id ? "border-lime-400 bg-lime-50" : "border-slate-200 hover:border-slate-300"}`}>
                      ...
                      {p.id !== "none" && (
                        isActiveOnAccount ? (
                          <p className="mt-2 text-xs font-semibold text-lime-700 flex items-center gap-1"><CheckCircle2 size={13} /> Membership active</p>
                        ) : (
                          <button
                            type="button"
                            onClick={() => startMembership(p.id)}
                          ...
                          >
                            {membershipActivating ? "Activating membership…" : "Start membership — demo"}
                          </button>
                        )
                      )}
                    </div>
                ...
                <p className="text-[11px] text-slate-400 mt-2.5">This is a V1 demo activation — no payment is taken yet. It sets your membership status so the Stockly team can follow up on billing.</p>
              </div>
            </div>
          )}
