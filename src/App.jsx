...
/* Demo/mock ops data has been removed. Operations dashboards use Supabase. */
...
/* ---------------------------------------------------------------------
   REAL OPERATIONS DASHBOARDS — Phase 11
--------------------------------------------------------------------- */
    ...
    <div className="bg-slate-950 border-b border-slate-800">
      <Section className="flex items-center justify-between h-16">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <button onClick={() => go("home")} className="text-slate-300 hover:text-white flex items-center gap-1.5 text-sm">
            <LogOut size={16} /> Exit
          </button>
        </div>
      </Section>
...
function AdminDashboard({ go, account }) {
  ...
  const reload = async () => {
    ...
    try {
      ...
      setError(err?.message || "Couldn't load the operations board.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <Section className="py-8">
          <div className="flex items-start justify-between gap-4 flex-wrap mb-6">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={reload} disabled={loading} className={btnPrimary + " !py-2.5 text-xs"}>{loading ? "Refreshing…" : "Refresh"}</button>
            </div>
          </div>
          ...
          <div className="flex gap-2 mb-5 overflow-x-auto stockly-scroll pb-1">
            {[['all','All runs'], ['active','Active'], ['unassigned','Needs assignment']].map(([key, label]) => (
              <button key={key} onClick={() => setFilter(key)} className={`shrink-0 px-4 py-2 rounded-full text-xs font-semibold border transition ${filter === key ? "bg-slate-950 text-white border-slate-950" : "bg-white text-slate-600 border-slate-200 hover:border-slate-300"}`}>{label}</button>
            ))}
          </div>
                  ...
                  <div key={order.dbId} className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                    <div className="p-5">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-4">
                        <button disabled={busy || order.status === "Requested" || order.status === "Cancelled"} onClick={() => changeStatus(order, -1)} className={btnGhost + " !py-2 !px-3 text-xs"}><ChevronLeft size={13} /> Previous</button>
                        <button disabled={busy || ["Delivered", "Cancelled"].includes(order.status)} onClick={() => changeStatus(order, 1)} className={btnPrimary + " !py-2 !px-3 text-xs"}>Next <ChevronRight size={13} /></button>
                        <button disabled={busy || ["Delivered", "Cancelled"].includes(order.status)} onClick={() => cancel(order)} className="px-3 py-2 rounded-xl text-xs font-semibold text-red-600 border border-red-100 hover:bg-red-50">Cancel</button>
                        <button onClick={() => setExpandedId(expanded ? null : order.dbId)} className="ml-auto px-3 py-2 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-50">{expanded ? "Hide details" : "View details"}</button>
                      </div>
                    </div>
          ...
          {!openOrder ? (
            <>
              <div className="flex items-end justify-between gap-3 mb-6">
                <div><p className="text-xs font-semibold text-lime-700 uppercase tracking-wider">Your queue</p><h1 className="font-display font-bold text-3xl text-slate-950 mt-1">Today's stock runs</h1><p className="text-sm text-slate-500 mt-1">{myOrders.length} assigned to you</p></div>
                <button onClick={reload} className={btnGhost + " !py-2.5 text-xs"}>Refresh</button>
              </div>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-2.5"><button disabled={busy || openOrder.status !== "Confirmed"} onClick={() => updateStatus(openOrder,"Purchasing")} className={btnPrimary + " !py-3"}>Start purchasing</button><button disabled={busy || !["Confirmed","Purchasing"].includes(openOrder.status)} onClick={() => updateStatus(openOrder,"Collected")} className={btnGhost + " !py-3"}>Mark collected</button></div>
              <button disabled={busy} onClick={() => uploadReceipt(openOrder)} className={btnGhost + " w-full mt-2.5 !py-3"}><Upload size={16}/> {openOrder.receiptUploaded ? "Receipt recorded" : "Mark receipt uploaded"}</button>
            </div>
          )}
        </Section>
      </div>
